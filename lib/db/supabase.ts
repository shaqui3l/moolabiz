...
// Convenience proxy – preserves call-site ergonomics
...
export async function getBusinessByWhatsApp(
...
export async function upsertBusiness(
  business: Partial<Business> & { whatsapp_number: string }
): Promise<Business | null> {
  const { data } = await supabase
    .from("businesses")
    .upsert(business, { onConflict: "whatsapp_number" })
    .select()
    .single();
  return data as Business | null;
}
...
export async function getOrCreateCustomer(
  ...
  const { data } = await supabase
    .from("customers")
    .insert({ whatsapp_number: whatsappNumber, business_id: businessId })
    .select()
    .single();
...
export async function createAppointment(
  appt: Omit<Appointment, "id" | "created_at">
...
export async function getUpcomingAppointments(
  ...
  const { data } = await supabase
    ...
    .lte("datetime", until.toISOString());
