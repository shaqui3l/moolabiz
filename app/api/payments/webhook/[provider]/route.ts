...
async function sendReceipt(orderId: string): Promise<void> {
  // Fetch customer phone and name for the order
  ...
  if (customer?.whatsapp_number) {
    ...
    await sendMessage(
      ...
      `✅ Payment received! Thank you ${name}. Your order is confirmed. We'll be in touch shortly.`
    );
  }
}
  ...
  try {
    if (provider === "yoco") {
      ...
      const payload = JSON.parse(rawBody) as {
        type: string;
        payload: { metadata: { orderId: string } };
      };
    ...
    if (provider === "ozow") {
      ...
      if (!verifyOzowWebhook(formParams)) {
        return NextResponse.json({ error: "Invalid signature" }, { status: 400 });
      }

      if (formParams.Status === "Complete") {
        const orderId = formParams.TransactionReference;
        await updateOrderStatus(orderId, "confirmed", "paid");
        await sendReceipt(orderId);
      }
    ...
    if (provider === "payfast") {
      ...
      if (!verifyPayFastWebhook(formParams)) {
        return NextResponse.json({ error: "Invalid signature" }, { status: 400 });
      }
